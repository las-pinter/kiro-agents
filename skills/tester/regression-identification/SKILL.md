...
1. **Trace dependents** — what calls or imports the changed code? Those are regression candidates
1. **Check existing tests** — do current tests cover the changed behavior? Run them
1. **Identify gaps** — what changed behavior is NOT covered by existing tests?
1. **Write new tests** — cover the gaps before marking the change complete. For deciding what type of tests to write, load `test-strategy-selection.md`
